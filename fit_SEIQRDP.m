...
p.addOptional('tolX',1e-7);  %  option for optimset
p.addOptional('tolFun',1e-7);  %  option for optimset
p.addOptional('Display','iter'); % Display option for optimset
p.addOptional('dt',01); % time step for the fitting
p.parse(varargin{:});
%%%%%%%%%%%%%%%%%%%%%%%%%%
tolX = p.Results.tolX ;
...
%         sprintf('function call')
    ...
    function [guess,kappaFun] = getKappaFun(tTarget,Q,D,guess)
        ...
        if max(D)<10
            ...
            try
                opt=optimset('TolX',1e-8,'TolFun',1e-8,'Display','off');
...
%                myFun1 = @(a,t) a(1).*exp(-a(2)*(t+(a(3))));
               ...
               myFun3 = @(a,t) a(1) + exp(-a(2)*(t+a(3)));
                ...
                rate = (diff(D)./median(diff(tTarget(:))))./Q(2:end); %% rate=((D(t2)-D(t1))/((t2-t1))*Q(t2))
                x = tTarget(2:end);
%                 plot(rate)
...
%                 myFun1 = @(a,t) (a(1)*exp(a(2)*(t-a(3))))./(1+(exp(-a(2)*(t-a(3)))/exp(a(2)*(t-a(3)))));
%                 myFun2 = @(a,t) (a(1)*exp(a(2)*(t+a(3))))./(1+(exp(-a(2)*(t+a(3)))/exp(a(2)*(t+a(3)))));
